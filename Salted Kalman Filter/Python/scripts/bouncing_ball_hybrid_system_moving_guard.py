...
def symbolic_dynamics():
    ...
    """ Define guards. """
    # x_p = 0
    # x_p = 0.25*sp.sin(4*np.pi*t) # moving paddle position as a function of time
    # gIJ = Matrix([q - x_p])
    gIJ = Matrix([q - 0.25*sp.sin(4*np.pi*t)])
    gJI = Matrix([q_dot])
    ...
    gIJ_func = sp.lambdify((time, states, inputs, dt, parameters), gIJ)
    GIJ_func = sp.lambdify((states, inputs, dt, parameters), GIJ)
    GtIJ_func = sp.lambdify((time, states, inputs, dt, parameters), GtIJ)

    gJI_func = sp.lambdify((time, states, inputs, dt, parameters), gJI)
    GJI_func = sp.lambdify((states, inputs, dt, parameters), GJI)
    GtJI_func = sp.lambdify((time, states, inputs, dt, parameters), GtJI)
