...
% Plot results
figure;
plot(actual_states(:, 1), 'k-', 'DisplayName', 'Actual states');
hold on;
plot(measurements(:, 1), 'r.', 'DisplayName', 'Measurements');
plot(filtered_states(:, 1), 'b--', 'DisplayName', 'Filtered states');
plot(guard, 'k--', 'DisplayName', 'Guard');
legend;
xlabel('Timestep');
ylabel('Position (y)');
title('1D Bouncing Ball Position');
hold off;
