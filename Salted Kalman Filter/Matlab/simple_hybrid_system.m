...
% Plot results1
figure;
plot(actual_states(:, 1), actual_states(:, 2), 'k-', 'DisplayName', 'Actual states');
hold on;
plot(measurements(:, 1), measurements(:, 2), 'r.', 'DisplayName', 'Measurements');
plot(filtered_states(:, 1), filtered_states(:, 2), 'b--', 'DisplayName', 'Filtered states');
legend;
xlabel('Position (x1)');
ylabel('Velocity (x2)');
title('2D State Space for Hybrid System');
hold off;
