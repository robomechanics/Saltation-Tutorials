...
%% First, choose mode to run %%%%%%%%%%%%%%%%%%%%%%%%%
%%% mode 1 == 1D bouncing
%%% mode 2 == Sliding
%%% mode 3 == Sticking

mode = 1;%%%THIS IS THE ONLY LINE YOU SHOULD CHANGE TO RUN PRESET DEMOS%%%%

if mode == 1
    addpath('Dynamics Files\Bouncing')
    filename = "bouncing";
elseif mode == 2
    addpath('Dynamics Files\Sliding')
    filename = "sliding";
elseif mode == 3
    addpath('Dynamics Files\Sticking')
    filename = "sticking";
else
...
rng(10); % fixing seed for reproducability, uncomment if you want truly random noise
...
if mode == 1
    % x_0 should be a two by one in modes 1: x = [q1; q1_dot]
    theta = 0;
    x = 2.0;
    x_dot = 0.0;
    k = 0.25; %noise sensitivity parameter

else
    % x_0 should be a four by one in modes 2 and 3: x = [q1; q2; q1_dot; q2_dot]
    x = 0.25;
    y = 5;
    x_dot = 0.5;
    y_dot = -0.125;
...
for i=1:num_sims
    ...
    if mode == 1
        x_0 = [x + dx*k ; x_dot + dx*k/5];
    else
        x_0 = [x + dx*k; y + dy*k*2; x_dot + dx*k/5; y_dot + dy*k/5];
    end

    [state_vec, Xi_vec, t_event, t_vec] = simulate(x_0,u,t_f);
...
[state_vec, Xi_1_vec, Xi_2_vec, t_event, t_vec] = simulate(x_0,u,t_f);
x_trajectory = state_vec(:, 1);
y_trajectory = state_vec(:, 2);
