...
for i=1:num_sims
    ...
    [state_vec, Xi_vec, t_event, t_vec] = simulate_stick(x_0,u,t_f);
...
x_0 = [x; y; x_dot; y_dot];
[state_vec, Xi_1_vec, Xi_2_vec, t_event, t_vec] = simulate_stick(x_0,u,t_f);
x_trajectory = state_vec(:, 1);
y_trajectory = state_vec(:, 2);
...
hold on
e1 = error_ellipse(naive_cov([1,2],[1,2]), state_vec(end, 1:2), 'conf', 0.9);
e2 = error_ellipse(current_cov([1,2],[1,2])' + eye(2)*0.0000001, state_vec(end, 1:2), 'conf', 0.9);
e1.LineStyle = '--';
e1.Color = orange;
e1.LineWidth = 2;
